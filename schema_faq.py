...
def generate_answers(dest: str, mode: str) -> list[tuple[str, str]]:
    ...
    prompt = f"""\
SEN: Yoldaolmak.com seyahat blog yazarısın.
...
def run_tests(dest: str, block: str, mode: str, pairs: list) -> bool:
    ...
    chk('Yazar: Kemal Kaya',                  '"name": "Kemal Kaya"' in block)
    chk('Yayıncı: Yoldaolmak.com',           '"name": "Yoldaolmak.com"' in block)
    chk('Logo URL var',                       'yoldaolmak.com/logo.png' in block)
